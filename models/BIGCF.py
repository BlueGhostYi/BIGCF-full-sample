"""
Exploring the Individuality and Collectivity of Intents behind Interactions for Graph Collaborative Filtering SIGIR‘23
"""
...
class BIGCF(nn.Module):
    ...
    def forward(self, user, positive, negative):
        ...
        reg_loss = utility.losses.get_reg_loss(ego_user_emb, ego_pos_emb, ego_neg_emb, self.user_intent.weight, self.item_intent.weight)
        reg_loss = self.reg_lambda * reg_loss

        ssl_user_loss = utility.losses.get_InfoNCE_loss(user_embedding, user_embedding, self.ssl_temperature)

        ssl_pos_loss = utility.losses.get_InfoNCE_loss(pos_embedding, pos_embedding, self.ssl_temperature)

        ssl_inter_loss = utility.losses.get_InfoNCE_loss(user_embedding, pos_embedding, self.ssl_temperature)

        ssl_user_loss_2 = utility.losses.get_InfoNCE_loss(intent_user_embeddings[user.long()], intent_user_embeddings[user.long()], self.ssl_temperature)

        ssl_pos_loss_2 = utility.losses.get_InfoNCE_loss(intent_item_embeddings[positive.long()], intent_item_embeddings[positive.long()], self.ssl_temperature)
